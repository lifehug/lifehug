...
def keystones(data: dict, n: int = KEYSTONE_CAP) -> list[dict]:
    ...
    v198 (ADR 0026): ordering independently by
    ...
    **zero** — two questions that place exactly what one question places.
    ...
    v204: the scoring pass and the greedy loop moved out to `_scored_anchors`
    and `_greedy_plan`. (The Reading Room, ADR 0025, retired 2026-09-03,
    extended this same plan to `k` picks with a witness partition; nothing
    about a keystone changed by its removal.)
...
# arithmetic. Beside `keystones`, the house home for derived
# timeline arithmetic.
# ---------------------------------------------------------------------------
...
def _greedy_plan(scored: list[dict], n: int) -> list[dict]:
    """The one greedy-over-the-residual definition (ADR 0026).

    Every unknown weighs 1.0; the key is `(-gain, cost, anchor)`.
    """
    plan: list[dict] = []
    covered: set[str] = set()
...
# ---------------------------------------------------------------------------
# unknown_width — the continuous width-sum ranking quantity (v204, ADR 0027).
# Beside `keystones`/`placement_score`, the house home for derived timeline
# arithmetic. (The Reading Room, ADR 0025, retired 2026-09-03, was v204's
# other user of this quantity; the placement score is unaffected.)
# ---------------------------------------------------------------------------
...
def unknown_width(row: object) -> float:
    ...
    a width-sum degenerates exactly to marginal coverage when no unknown
    carries bounds.
    """
    if not isinstance(row, dict):
        return 1.0
