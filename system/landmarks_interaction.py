...
#: Where the claims a date OUTRANKED are kept (v221, B4). Never overwritten,
...
def merge_landmark_entry(existing: object, record: object) -> dict:
    ...
    **The DATE fields are the exception to the dict merge** (v221, B4).
...
def landmark_date(entry: object, *, bound: object = None) -> dict:
    ...
    conflict; today the point is that both are ANSWERABLE — before v221 the
...
def landmark_invocation(record: object) -> list[str] | None:
    ...
    v221 (B4): a date crosses to the writer WHOLE. This function used to
...
def date_flags(flag: str, record: object) -> list[str]:
    ...
    **This is the PERSON-ROSTER carriage, and it is narrower than the landmark
    one on purpose — for now.** v221 made `chronology.date_argv` /
