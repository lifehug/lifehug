...
#: Where the claims a date OUTRANKED are kept (v222, B4). Never overwritten,
...
def landmark_date(entry: object, *, bound: object = None) -> dict:
    ...
    conflict; today the point is that both are ANSWERABLE — before v222 the
...
def landmark_invocation(record: object) -> list[str] | None:
    ...
    v222 (B4): a date crosses to the writer WHOLE. This function used to
...
def date_flags(flag: str, record: object) -> list[str]:
    ...
    **This is the PERSON-ROSTER carriage, and it is narrower than the landmark
    one on purpose — for now.** v222 made `chronology.date_argv` /
