...
def cmd_prompt(args):
    ...
    lines.append("  0. SELF-CONTAINED RULE (MOST IMPORTANT): the question will fire STANDALONE")
    lines.append("     as a daily prompt, sometimes months later. It must read correctly with")
    lines.append("     ZERO surrounding context. NAME every person, place, and thing — never")
    lines.append("     use bare pronouns ('her', 'him', 'them', 'it', 'that') whose referent")
    lines.append("     lives only in a previous answer. WRONG: 'You said you wish you could talk")
    lines.append("     to her now...' RIGHT: 'You said you wish you could talk to your grandma")
    lines.append("     now...' If the antecedent is unclear from THIS question alone, the")
    lines.append("     question is broken. Read your draft as if you'd never seen the parent Q.")
    lines.append("  1. TWO-SENTENCE RULE: one sentence of context quoting the author's own words,")
    lines.append("     then ONE open question. One question mark per follow-up.")
    lines.append("  2. Specific, not general — 'Tell me more about the blue Toyota' not 'Tell me about cars'")
    lines.append("  3. Sensory — 'What did that place look like? Sound like? Smell like?'")
    lines.append("  4. Emotional — 'What were you feeling in that moment?'")
    lines.append("  5. Dialogue — 'What did she say? What did you say back?'")
    lines.append("  6. Before/after — 'What changed after that?'")
    lines.append("  7. Contrast — 'How was that different from what you expected?'")
    lines.append("  8. Never yes/no — always open-ended. 'What'/'When'/'Tell me about', never 'Why',")
    lines.append("     for the author's own feelings ('why' is fine for events and other people).")
    lines.append("  9. Five-slot check (McAdams): if a scene is missing what happened / when & where /")
    lines.append("     who was there / what they thought & felt / WHAT IT SAYS ABOUT THEM — target the")
    lines.append("     empty slot. The last one is the highest-value follow-up there is.")
    lines.append("  10. Action↔identity ladder: after an action, ask what it says about them;")
    lines.append("      after an identity claim, ask for one specific moment that proves it.")
    lines.append("  11. NEW ANGLES ONLY on depth passes: ask for what's never been told")
    lines.append("      ('a detail from that day you've never mentioned to anyone'), never a")
    lines.append("      re-rehearsal of the canonical version.")
    lines.append("  12. Never restate the author's account with changed details — quote exactly.")
    lines.append("")
