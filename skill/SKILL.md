...
7. Chats close themselves: `conversation-close --expired` (v156, issue #119) is the deterministic, AI-free idle sweep — it finds every open session past its idle timeout and enqueues one durable `conversation-close` job per session rather than closing inline; `compile_and_commit.sh` runs it hourly. A chat with at least two of the user's turns gets a closing takeaway; a partial one closes SILENTLY — never nag someone for stopping. `conversation-close <session_id>` (no `--expired`) runs the FULL close on demand: takeaway-or-silence, then files the Mirror inbound + engagement timing, compiles the wiki ONCE, and lands ONE commit — non-fatal on git errors.
8. `process-answer` compiles the wiki automatically by default. Use `--no-compile-wiki` only for tests or emergency repairs. **Exception (v156, issue #119):** an answer filed inside an OPEN conversation session skips the compile by default instead — the session's eventual close is the batch boundary that compiles once and commits once for the whole exchange. `--compile-wiki` forces the compile anyway.
9. Commit only when the user asks, or when operating an explicit daily/cron workflow — inside an open session, `--commit`/`--push` still only fire on explicit request; per-answer commits stay off by default there too, same as everywhere else.
