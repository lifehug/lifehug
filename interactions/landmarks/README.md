...
| The date/span shapes both leaves' own examples teach (v290) | `conversation_delivery._parse_landmark_date(raw)` accepts a bare string (bounded by `_PLACED_TEXT_MAX_CHARS`) as well as the `{"best", "earliest", …}` dict — wrapped as `{"best": raw}`, the SAME shape a minimal model dict already produced, so `span`'s two bounds (which route through the same function) are covered for free. `chronology.parse_loose_date(text)` reads the forms both leaves now name — a year, a year-month, a full date, or a month name (or its 3-letter abbreviation, case-insensitive) with a year — into the identical dict `chronology.normalized_date` returns; a whole date bracketed (`[Jun 1986]`) is the owner's own estimation convention and comes back `confidence: "approximate"`. `normalized_date` falls back to it wherever EDTF alone cannot read a `best` string. Every existing dict shape, and what `basis` a model date gets, is unchanged |
