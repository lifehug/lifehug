...
package's request-key, current-identity/reuse and rank helpers (ADR 0033 amendment
5). A house has its own roster place below its city; repeated stays share that
place, not an episode. This supersedes the historical v292 city-alias wording
below. Alias collisions stay explicit; undo respects surviving owners. Settled
house identity and hierarchy survive canonical roster refresh, including empty
or conflicting model output. Already applied receipts are never auto-migrated.
...
**Amended 2026-09-04 — R6–R9, the reading** (`add-landmark-reading-plan.md` §2, ADR 0033's Cut 6f amendment, v291). The owner's first real use of Add Landmark — a thirty-stay residence history pasted in — read wrong in five verified ways, and the cause was the ORDER: a deterministic block grammar ran in front of the interaction and hid its work from it. **R6: the interaction reads; the system validates and files.** Add Landmark's `offer` path is now ONE model pass over the whole submission with the whole context — `interactions/landmarks/prompt/reading.md` and `system/landmark_reading.py` — and no deterministic reader touches the person's text before it. `grammar_units`, the listener and the recorder are off this path (`collect` mode is untouched and still uses both). **R7: a span is the unit of relation** — a school or job named inside a stay carries `within` and inherits the stay's dates as a stated inference with a verbatim provenance clause. **R8: estimation marks (brackets, "about", "?") are the interaction's convention** and map to `approximate` on one bound. **R9: one reading per submission** — a long paste is never split, sampled or timed out early. A unit with no date read is `basis: "none"`, never "inferred". Hosts thread four new things: `within` and `names` on a unit, `events[]` on the proposal, and the `reading` completion key (`host_reading_prompt` replaces `host_listener_prompt`; `host_recorder_prompts` and `--listener-completion` are deleted). **Amended again 2026-09-04 — R7's filing** (ADR 0033 amendment 2, v292). The proposal is GROUPED: `groups[]` is one entry per top-level unit in text order, its `members` the units, events and stories whose `within` resolves to it transitively, with a trailing `{unit_id: null}` entry for what belongs to nothing — a thirty-block document reads as thirty cards. Confirming a stay files what is on its card: the nickname as a roster ALIAS on the place its city minted (the place join is provable source overlap, never a keyword), the city and address as the place's names, an inherited span with its provenance clause intact, a dated event as a claim, an undated one as an `occurrence` moment, and one promoted SLICE per group — the stay's own words — as the source those cite. The slice carries `question_context` = the stay's telling ref, which `bind-episodes` turns into a deterministic `part_of`; `apply` stamps and the binder files. `retract` undoes all of it, alias included, and deletes nothing. Episode-in-episode containment is NOT a relation the calculated graph holds (its relation block is telling→episode), so a school inside a stay is a date inheritance and not a rendered edge. **Amended again 2026-09-04 — Cut 6h (v293): the grammar leaves the path.** `grammar_units`, `_date_dict` and `_grammar_block_quote` are deleted from `landmark_offer.py`, and with them the module's only `go_dig_writer.plan_import` import — `propose` already read one way (R6); this cut removed the dead code that could still have called back into `go_dig_grammar`. `apply`'s `go_dig_writer.record_unit` writer seam is untouched and is now the ONLY `go_dig_writer` name the module carries outside `unit_filing_digest`'s pre-existing filing-identity helper. The host-run reading protocol (one prompt, one `{"reading": …}` completion) is unchanged; `go_dig_grammar.py`/`go_dig_writer.py` themselves are Cut 7b's to delete.
