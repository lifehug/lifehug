...
`focus-new` scaffolds a new question-bank category, registers the Focus on the roadmap, and **auto-generates ~8–12 starter questions** toward the objective. Generation uses the shared provider (direct local model, OpenClaw, explicit Kimi/Anthropic); if it is unavailable, the Focus is still created and you generate the questions yourself (next section). A configured local route never leaks into cloud fallback, and the optional Anthropic SDK is not required for the keyless path. It never touches existing answers.
