...
- **Anchors** — the superset of **cornerstones** (the fixed dated points) and **landmarks** (the spans): everything the person gives that the timeline hangs on. Owner-named as the product word 2026-09-25; it was already the code term for a landmark's dated result. **The spine is age frames plus anchors**: a fresh vault has only the age frames a birthday creates, and each anchor makes it more specific. Keystones are the anchors worth asking for next; whispers and keystone questions ask for them. See [The Spine](docs/handbook/the-spine.md).
...
The timeline's vocabulary is one named set — terms that only make sense
together (owner-set, 2026-08-23; **cornerstone** and **anchors** added 2026-09-25).
...
- **Cornerstone** — one of the small fixed set of dates a life is measured from, the same for everyone and the only dates asked to the **day** (owner-named, 2026-09-25): your birth; each child's birth; your parents', siblings' and spouse's births; each of your weddings and divorces; the deaths of your parents, spouse, siblings, children and grandparents; your baptism once you mention it (`system/cornerstones.py`). Everything else is asked no finer than a **month**, however much it would place — leverage makes a card sooner, never finer (`temporal_work_items.A_DAY_IS_ASKED_ONLY_OF_A_CORNERSTONE`). A cornerstone-type event for someone outside the set (*"When did your sister get divorced?"*) is asked once and accepted at whatever grain you give. A day worked out by arithmetic is shown as its month. A wedding is a cornerstone; the marriage it opens is a span that stays open while you are married. ADR 0038.
...
**How they relate:** anchors are cornerstones plus landmarks, and the spine is age frames plus anchors; landmarks are the universal skeleton — spans and points,
by domain; keystones are the computed highest-leverage gaps that skeleton
leaves right now; cornerstones are the fixed dated points everything is
measured from, asked to the day while everything else stops at a month; and
whispers and keystone questions are the two ways the loop asks.
...
- **Revisit, aim, estimate** (v325). A newly filed story re-opens the settled unknowns it bears on, once each. An answer may return `handle_binds` — *"this moment's unresolved handle names THAT node"* — which files a `relative_order` claim and retires the raw handle, so the `missing_anchor` card goes away because nothing is missing. And when the vault genuinely cannot tell, the answer carries a mechanically-verified **`probable_window`**: published on the node and its card, drawn as a floating dot whose height is the window's width. Since v360 (owner, 2026-09-25) a grounded window of about five years or less is filed as the system's own inferred placement (`resolver.AN_ESTIMATE_PLACES_AS_THE_SYSTEMS_INFERENCE`), always ranked below anything you said and never retiring a word of yours (`resolver.AN_ESTIMATE_NEVER_RETIRES_HIS_WORDS`); a wider one stays the floating dot and is asked about only when it is hot — a keystone, or a moment others wait on (`resolver.AN_ESTIMATE_WIDER_THAN_FIVE_YEARS_STAYS_A_WINDOW`, `temporal_publication.A_WIDE_ESTIMATE_IS_ASKED_ONLY_WHEN_HOT`). An answer whose end is empty on a stretch ("still", "throughout") is read as ongoing, an open span, never as one day (`resolver.AN_ANSWER_WITH_NO_END_IS_ONGOING`). Since v361 an estimate is not guessed again: a moment placed by the system's own estimate is re-opened only by a new story that carries a date, or by your answer to its card (`resolver.AN_ESTIMATE_IS_REVISITED_ONLY_BY_A_DATED_STORY`), and a different guess from the same undated story files nothing (`resolver.A_GUESS_NEVER_REPLACES_A_GUESS`).
