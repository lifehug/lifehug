...
- **Landmarks** — the **universal** set of dating questions everyone gets: birth, the places lived, schooling, partnerships, children, jobs. They are the same for every person, and they are the skeleton that makes everything else placeable by arithmetic — knowing a birthday and a stated age *is* a year; knowing when you lived somewhere dates every moment that happened there. Some are answered at onboarding; the rest sit under the Timeline with Play and surface later. In code the *dated results* of these questions are the **anchors** every probe hangs on (`timeline.anchor_index`, `chronology.from_age` / `from_anchor`). The research note `system/research/landmarks.md` is the authority for the final set and its wording.
...
- **Whisper** — information woven into a conversation that fits naturally, drawn from an arc that has developed, and serving a **second agenda beyond the conversation's primary one**. Whispers are general; the **timeline whisper** is the first kind — a keystone's real probe and the person's own landmarks, raised only where it fits, at most once per conversation, any precision accepted, never pressed.
