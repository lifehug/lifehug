...
| **`process_answer.py`** | The answer pipeline. Saves the answer to `answers/<id>.md`, marks the question done, rebuilds coverage, updates rotation, refreshes the README, recompiles the wiki, silently scores richness, then best-effort sends a warm Telegram acknowledgment before any adaptive follow-up. The one command that runs after every reply. |
