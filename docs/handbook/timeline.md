...
| What an answer to a card means | `system/answer_placement.py` (`ANSWERING_A_CARD_PLACES_ITS_MOMENT`, `work_item_of_session`, `card_for_answer`, `answer_reading`, `aim_at_card`, `place_answers`); seats `classifier_claims.migrate_classifier_moments` and `landmark_recorder.file_claims`; verb `place-answers [--dry-run] [--source PATH]` |
