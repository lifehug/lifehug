...
| The warrant a date carries (v219) | `chronology.date_argv` / `date_from_argv`, `WARRANT_FIELDS`, `date_flag_names`; `lifehug.py landmark-record --basis/--granularity/--confidence/--anchor/--provenance`, and the same five under `--start-` and `--end-` |
| Two dates for one entry (v219) | `merge_landmark_entry` → `chronology.reconcile`; `landmarks_interaction.DATE_ALTERNATES_KEY`, `SPAN_ALTERNATES_KEY`, `landmark_date`; `chronology.merge_claims`, `conflict_strength` |
