...
| **Birth is the owner's** (v339) | the `birth` domain is the owner's own birthday and nobody else's — it is the one domain with no identity rung, so a relative's birth filed here keys on the same empty `entry_key` and folds into the owner's entry. A record that names somebody who is not the owner, or that carries a year fifteen or more from the year the owner STATED, is routed to `family` under their name or refused by name (`landmark_projection.birth_landmark_not_owner` → `birth_landmark_not_owner`, the fourth `NOT_A_LANDMARK_REASONS` member). Refused at FILING by `timeline.save_landmark`, skipped at DRAW by `project_landmark_entries`. Two pasted grandfathers' vital records had become the owner's own `date_alternates` and asked him which of 1981 and 1929 was his birth (ADR 0037 amendment v339) |
| **A birth record's own domain words name nobody** (v341) | the second half of the same rule, and the half v339 got wrong. `birth` has no identity rung, so the hosts file the owner's own birthday under the domain's natural DISPLAY label — `{"domain": "birth", "label": "Born", "date": {"best": "1979"}}` — and v339 read "Born" as the name of a third party and dropped the owner's own birth landmark from the drawing (four hosted-platform tests failed on the v339/v340 pin, one of them asserting the literal `['Born']` survives). `landmark_projection.BIRTH_DOMAIN_WORDS` / `is_birth_domain_word` is the ONE definition of that vocabulary — `birth`, `born`, `birthday`, `birthdate`, `birth date`, `date of birth`, `my`/`your`/`owner's birth`, `i was born` — compared WHOLE and casefolded, never as a substring, so "Mary Born" and "Bornstein" are still names and still refuse. Both seats that ask the question read that set: the landmark's `third_party_birth_subject` and the projection's age anchor (`temporal_timeline._birth_names_only_the_owner`, which kept its own single-word list until v341) |
| **A landmark is drawn as what it IS** (v345) | `date_event_kind` answers for a DOMAIN, and that is right for eight of the nine. `family` is the one whose entries are not all the same shape: it declares `date_semantics: birth` because a sibling entry dates a sibling's birth, and the owner's own vault holds ONE `family` entry — `{"label": "parents", "who": "parents", "relation": "parent", "date": "1976-06-25"}`, his parents' WEDDING — which was therefore drawn as **"Parents's birth"**, a birth nobody has ever had, beside the "Parents' wedding date" node carrying the same 1976-06-25. A couple is not born. `landmark_projection.entry_date_event_kind` is the ONE definition of what an ENTRY's date dates (`A_LANDMARK_IS_DRAWN_AS_WHAT_IT_IS`): a declared `birth` is refined only three ways — the entry STATES a birth of its own (`ENTRY_BIRTH_FIELDS`) and stays one; its own words name a marriage (`MARRIAGE_DATE_WORDS`) or its subject is exactly ONE couple (`COUPLE_SUBJECT_WORDS`, whole-text and casefolded exactly as `is_birth_domain_word` is) and it is `married`; a collective that is not one couple (`COLLECTIVE_SUBJECT_WORDS` — `grandparents`, because a person has up to four of them and two of their weddings — or any enumeration of two or more names) is `transition`, this module's existing word for a date whose event the record does not say. Read at THREE seats — `entry_claims` at filing, `_attach_dates` and the fold's `read_landmark_dates` at draw — so a vault that already holds the record heals on its next redraw with no migration, and the id it used to be drawn at is REDIRECTED rather than lost (`temporal_timeline._landmark_redraw_aliases`, `landmark_date_kind_redrawn`) |
| **A telling of a landmark folds onto it** (v345) | the other half of the same review. A landmark node is a legitimate target for the binder's milestone rung, and two readings were missing before it could reach one: `episode_binder.MILESTONE_OF_EVENT_NOUN` reads `wedding` out of a LABEL that carries no verb (`marriage` and `anniversary` are deliberately absent — a marriage is a state a life spends years inside, and the owner's vault holds nine nodes that say so; a noun after a preposition is an adjunct, so "Father fell ill after wedding" is an illness), and `landmark_projection.names_a_birth` reads the three shapes a birth is said in out of the EVIDENCE — so "Harvey arriving", typed `moment` by the classifier with the quote *"Birth of Harvey during the Etherfuse chapter"* behind it, is Harvey's birth. The name the quote gives must be somebody the telling is already about, the classifier's own when-metadata is cut first (`BIRTH_PHRASE_WHEN_MARKERS`) and a birth introduced by a temporal preposition is an adjunct (`BIRTH_PHRASE_IS_AN_ADJUNCT_AFTER`) — *"shortly after James's birth"* dates a flight and is not one. And a marriage is once per COUPLE (`identity_resolution.ONCE_PER_COUPLE_EVENT_KINDS`, `couple_key`): "Mom married dad at 21" names `mother`, "Parents' wedding date" names `parents`, and only a couple key makes them one fact. `grandparents` is absent from `COUPLE_OF_RELATION_WORD` for the reason `graduation` is absent from `ONCE_PER_SUBJECT_EVENT_KINDS` — a person has up to four of them and two of their weddings |
| **A couple is two people, not one relationship word** (v350) | v345's own rule finishing its sentence, and the owner's vault paid for the gap. A roster introduction had filed `mother` as an alias of `person/desiree-taylor`; `episode_containers.resolve_entities` matches a contiguous TOKEN RUN, and `mother-in-law` tokenizes to `(mother, in, law)` — so *"Wedding reception in mother-in-law's backyard"*, the OWNER's reception on his own anniversary 2007-01-11 with subject `self`, resolved to HIS MOTHER, and `R2b`'s per-PERSON milestone bucket folded it into his parents' 1976-06-25 wedding beside *"Mom married dad at 21"*. Two legs, one seat each (`episode_binder.A_COUPLE_IS_TWO_PEOPLE`). A compound relationship word is never the simple word inside it (`identity_resolution.A_COMPOUND_RELATION_IS_NEVER_THE_WORD_INSIDE_IT`, `_not_a_simple_relation` — v347's `_NOT_A_POSSESSOR` in the other direction): a run that IS one relationship word is refused when an in-law suffix follows it (the one `IN_LAW_RE`, moved to sit beside the vocabulary it reads and re-exported from `axis_membership`) or a `COMPOUND_RELATION_PREFIXES` word precedes it — step, grand, great, half, god, foster, adoptive, ex, former, late — while a LONGER key is the compound itself and still matches. And a couple key is read from the telling's own SUBJECTS (`couple_key`, `TellingView.subject_mentions`) rather than from every person token in it, so the owner's own subject — `self`, `me`, `narrator`, and the first-person plural `we`/`our` (`OWNER_COUPLE_SUBJECT_WORDS`) — names the OWNER's couple and a word in a backyard names nobody. v345's vocabulary is unchanged, and under a resolved roster the subject-side reading RESTORES its pair: the introduction had already turned the age telling's person tokens into `{desiree, taylor}` |
...
| **Cornerstone** | one of the fixed dates asked to the DAY (owner-named, 2026-09-25; ADR 0038): the owner's birth, his children's, parents', siblings' and spouse's births, his weddings and divorces, the deaths of his parents, spouse, siblings, children and grandparents, his baptism once mentioned. The ladders' own grain is unchanged (`children` and `partnerships` still stop at `month`), and the day is asked by the Timeline's cornerstone card, not by a ladder rung. Nothing else is ever asked below a month |
