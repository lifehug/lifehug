...
- **Cornerstone** — one of the small fixed set of dates everything is measured from, asked to the day: your birth, your children's, parents', siblings' and spouse's births, your weddings and divorces, the deaths of your parents, spouse, siblings, children and grandparents, and your baptism once mentioned. Everything else is asked no finer than a month; a day worked out by arithmetic is shown as its month; a marriage is a span that stays open while you are married. Landmarks are the skeleton, keystones the computed gaps, cornerstones the fixed points.
- **Anchors** — cornerstones plus landmarks: everything you give that the timeline hangs on. The spine is the age frames your birthday creates, plus your anchors.
- **Place mention** — a place you name in a story ("in Arizona", "at Williams") ties to the stays you gave there and bounds the moment; whatever else you said about it narrows that, and it is never drawn across a gap between your stays. A house you lived at twice is both stays, minus the time between.
- **Relative place** — "dad's house", "mom's place": whichever home that person had at the time of the story, never a residence of its own.
...
`classify_story.py` is the structured-understanding pass. It reads immutable answer/source files and writes derived records under `state/classifications/`: people, places, periods, themes, contradictions, possible outputs, Focus opportunities, self-understanding insights, candidate follow-up questions, direct event dates, and source-grounded `within`/`before`/`after` relations to supplied canonical timeline candidates. Responses echo the source revision and stable context digest they saw, cite exact unique source text, and are rejected before any write if source or context changed. Weekly maintenance classifies a capped batch (`LIFEHUG_WEEKLY_CLASSIFY_LIMIT`, default `5`), migrates accepted events into temporal claims, then promotes candidates. A whole-archive pass uses `classify-story --batch-plan --limit 500 --skip-candidates` and files one schema-v1 envelope with `--from-batch-response`; finite callers can page past attempted refusals using exact source-plus-snapshot identities in `--exclude-items-json`, while changed identities become eligible again. Context-only items refresh events while preserving all other fields, exact-current items need no model call, and content-free replay receipts live under `state/classification_batches/`. The archive-only flag omits candidate instructions instead of generating and discarding questions; ordinary new stories still generate them. `classify-story --refresh-targets --limit 50` remains the normal bounded maintenance view. Published calculated nodes carry canonical `usable_placement: true|false`; clients consume it directly. Weekly dry-run previews the candidate promotion gate before the real job mutates the candidate store or question bank.

Since v306, one read-only canonical fold derives classifier candidates from
independent evidence. Publishing classifier output cannot change that context.
